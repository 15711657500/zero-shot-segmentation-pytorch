...
decay=0.0001
gpus=0
classifier="./ckpts/classifier.pth.tar"
...
python -W ignore train.py --backbone $backbone --dataset $dataset --model $model --norm $norm --batch-size $bs --momentum $momentum --weight-decay $decay --gpu-ids $gpus --classifier $classifier --workers 1 #--no-val --ratio $ratio #--resume $resume
