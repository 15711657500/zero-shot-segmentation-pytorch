...
class Decoder(nn.Module):
    def __init__(self, num_classes, backbone, BatchNorm, abn=False):
        ...
        self.relu = nn.ReLU()
        self.last_conv = nn.Sequential(nn.Conv2d(304, 300, kernel_size=3, stride=1, padding=1, bias=False),
                                       BatchNorm(300),
                                       nn.ReLU(),
                                       nn.Dropout(0.5),
                                       nn.Conv2d(300, 300, kernel_size=3, stride=1, padding=1, bias=False),
                                       BatchNorm(300),
                                       nn.ReLU(),
                                       nn.Dropout(0.1),
